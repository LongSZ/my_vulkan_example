...
void MyVulkanManager::init_glfw_window() {
    ...
    s_pWindow = glfwCreateWindow(screenWidth, screenHeight, "Titile", NULL, NULL);
    if (!s_pWindow) {
...
void MyVulkanManager::init_vulkan_instance() {
    ...
    inst_info.enabledExtensionCount = static_cast<uint32_t>(instanceExtensionNames.size());//扩展的数量
...
void MyVulkanManager::enumerate_vulkan_phy_devices() {
    ...
    LHW_INFO("[Vulkan硬件设备数量: " << gpuCount);
...
void MyVulkanManager::create_vulkan_devices() {
    ...
    LHW_INFO("[Vulkan硬件设备0支持的队列家族数量: " << queueFamilyCount << "]");
...
void MyVulkanManager::create_vulkan_swapChain() {
    ...
    if (queueGraphicsFamilyIndex == UINT32_MAX || queuePresentFamilyIndex == UINT32_MAX) {
        LHW_ERROR("there is not queue family index support Graphis or Present");
...
void MyVulkanManager::create_frame_buffer() {
    ...
    for (i = 0; i < swapchainImageCount; i++) //遍历交换链中的各个图像
    {
        ...
        assert(result == VK_SUCCESS);//检查是否创建成功
        LHW_INFO("[create frame buffer " << i << " sucess]");
    }
}
...
void MyVulkanManager::createFence() {//创建用于等待指定任务执行完毕的栅栏
    ...
    VkResult result = vkCreateFence(device, &fenceInfo, NULL, &taskFinishFence);//创建栅栏
    assert(result == VK_SUCCESS);
}
...
void MyVulkanManager::doVulkan() {
    MyVulkanManager::init_glfw_window();
