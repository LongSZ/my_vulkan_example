...
void ShaderQueueSuit_Common::create_uniform_buffer(VkDevice &device,
    ...
    if (flag) { 
        LHW_INFO("确定内存类型成功 类型索引为 " << alloc_info.memoryTypeIndex); 
}
    else { 
LHW_ERROR("确定内存类型失败!"); 
}
...
void ShaderQueueSuit_Common::create_pipeline_layout(VkDevice &device) {
    ...
    VkResult result = vkCreateDescriptorSetLayout(device, &descriptor_layout, NULL,
...
void ShaderQueueSuit_Common::create_shader(VkDevice &device) {
    std::string vertStr = FileUtil::loadAssetStr("shaders/commonTexLight.vert");//加载顶点着色器脚本
    std::string fragStr = FileUtil::loadAssetStr("shaders/commonTexLight.frag");//加载片元着色器脚本
    ...
    VkResult result = vkCreateShaderModule(device, &moduleCreateInfo, NULL,
    ...
    result = vkCreateShaderModule(device, &moduleCreateInfo, NULL,
...
void ShaderQueueSuit_Common::create_pipe_line(VkDevice &device, VkRenderPass &renderPass) {
    ...
    VkResult result = vkCreatePipelineCache(device, &pipelineCacheInfo, NULL,
                                                &pipelineCache);//创建管线缓冲
    assert(result == VK_SUCCESS);//检查管线缓冲创建是否成功
    result = vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineInfo, NULL,
                                           &pipeline);//创建管线
