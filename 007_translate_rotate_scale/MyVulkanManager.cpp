...
void MyVulkanManager::enumerate_vulkan_phy_devices()
{
    ...
    LHW_INFO("Vulkan硬件设备数量为 " << gpuCount);
...
void MyVulkanManager::create_vulkan_DepthBuffer()
{
    ...
    {
        LHW_ERROR("not support VK_FORMAT_D16_UNORM");
    }
...
void MyVulkanManager::initMatrix() {
    ...
    MatrixState3D::setProjectFrustum(-ratio * 0.8f, ratio * 1.2f, -1, 1, 10, 100);
}
...
void MyVulkanManager::drawObject()
{
    ...
    MatrixState3D::pushMatrix();										//保护现场
    MatrixState3D::translate(6.0f, 0, 0);									//沿x方向平移6
    MatrixState3D::rotate(30, 0, 0, 1);									//绕z轴旋转30°
    objForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0])); 						//绘制变换后的立方体
    MatrixState3D::popMatrix();										//恢复现场
